...
void Animation::render()
{
    ...
    if (pal->getCounter((uint32_t)MASK::FIRE_FAST) < _fireFastTextures.size())
        Game::getInstance()->renderer()->drawTexture(_fireFastTextures.at(pal->getCounter((uint32_t)MASK::FIRE_FAST)), x(), y(), frame->x(), frame->y(), frame->width(), frame->height());

    if (pal->getCounter((uint32_t)MASK::FIRE_SLOW) < _fireSlowTextures.size())
        Game::getInstance()->renderer()->drawTexture(_fireSlowTextures.at(pal->getCounter((uint32_t)MASK::FIRE_SLOW)), x(), y(), frame->x(), frame->y(), frame->width(), frame->height());

    if (pal->getCounter((uint32_t)MASK::SLIME) < _slimeTextures.size())
        Game::getInstance()->renderer()->drawTexture(_slimeTextures.at(pal->getCounter((uint32_t)MASK::SLIME)), x(), y(), frame->x(), frame->y(), frame->width(), frame->height());

    if (pal->getCounter((uint32_t)MASK::SHORE) < _shoreTextures.size())
        Game::getInstance()->renderer()->drawTexture(_shoreTextures.at(pal->getCounter((uint32_t)MASK::SHORE)), x(), y(), frame->x(), frame->y(), frame->width(), frame->height());

    if (pal->getCounter((uint32_t)MASK::MONITOR) < _monitorTextures.size())
        Game::getInstance()->renderer()->drawTexture(_monitorTextures.at(pal->getCounter((uint32_t)MASK::MONITOR)), x(), y(), frame->x(), frame->y(), frame->width(), frame->height());

    if (pal->getCounter((uint32_t)MASK::REDDOT) < _reddotTextures.size())
        Game::getInstance()->renderer()->drawTexture(_reddotTextures.at(pal->getCounter((uint32_t)MASK::REDDOT)), x(), y(), frame->x(), frame->y(), frame->width(), frame->height());
}
